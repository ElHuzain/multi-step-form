...
const RadioButton = () => {
    ...
    return (
        <div className="w-full rounded-xl py-8 justify-center flex gap-5 items-center bg-secondary4 h-12">
            <span onClick={() => { planType === 'monthly' ? "" : dispatcher(UpdatePlanType()) }} className={planType === 'monthly' ? SelectedTextStyle : TextStyle}>Monthly</span>
            ...
            <span onClick={() => { planType === 'yearly' ? "" : dispatcher(UpdatePlanType()) }} className={planType === 'yearly' ? SelectedTextStyle : TextStyle}>Yearly</span>
        </div>
    )
}
